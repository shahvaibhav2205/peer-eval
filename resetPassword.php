...
$stmt = $db->prepare('SELECT resetToken, resetComplete FROM faculty WHERE resetToken = :token');
...
if(isset($_POST['submit'])){
	...
	if(!isset($error)){
		...
		try {
			...
			$stmt = $db->prepare("UPDATE faculty SET password = :hashedpassword, resetComplete = 'Yes'  WHERE resetToken = :token");
			$stmt->execute(array(
	    ...
	    <div class="col-xs-12 col-sm-8 col-md-6 mx-auto">
	    	...
	    	<?php if(isset($stop)){
				...
				<form role="form" method="post" action="" autocomplete="off">
					...
					<hr>
					<div class="row">
						<div class="col-xs-12 col-md-12><input type="submit" name="submit" value="Change Password" class="btn btn-primary btn-block btn-lg" tabindex="3"></div>
					</div>
				</form>
