...
if(isset($_POST['submit'])){
	...
	if(!isset($error)){
		...
		$stmt = $db->prepare('SELECT password, email FROM faculty WHERE email = :email');
		$stmt->execute(array(':email' => $_POST['email']));
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		try {
			...
			$stmt = $db->prepare("UPDATE faculty SET resetToken = :token, resetComplete='No' WHERE email = :email");
			$stmt->execute(array(
				':email' => $row['email'],
				':token' => $storedToken
			));
	...
	<div class="row">
	    ...
	    <div class="col-xs-12 col-sm-8 col-md-6 mx-auto">
			<form role="form" method="post" action="" autocomplete="off">
				...
				<hr>
				<div class="row">
					<div class="col-xs-12 col-md-12"><input type="submit" name="submit" value="Send Reset Link" class="btn btn-primary btn-block btn-lg" tabindex="2"></div>
				</div>
			</form>
		</div>
	</div>
